...
import { ChevronLeft, ChevronRight, Check, Loader2, Heart, Shield, Truck, RotateCcw, Award } from "lucide-react";
...
export default function ProductDetailPage({
  ...
  const badgeClass: Record<string, string> = {
    New: "dp-badge-new",
    Bestseller: "dp-badge-best",
    Limited: "dp-badge-lim",
  };
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .dp-layout {
          ...
          padding: 36px clamp(1.5rem, 6vw, 5rem) 80px;
          max-width: 1400px;
          margin: 0 auto;
        }
        ...
        .dp-img-prev, .dp-img-next {
          ...
          color: #333; transition: background 0.25s, border-color 0.25s;
          z-index: 2;
        }
        .dp-img-prev { left: 14px; }
        .dp-img-next { right: 14px; }
        .dp-img-prev:hover, .dp-img-next:hover {
        ...
        .dp-enquire:hover:not(:disabled) { background: #b18d2b; letter-spacing: 0.38em; }
        .dp-enquire:disabled { opacity: 0.65; cursor: default; }
        .dp-enquire-done { background: #2d6e45 !important; }
        .dp-wish {
        ...
        .dp-strip {
          ...
          padding: 16px 18px; border-bottom: 1px solid #ece8e1;
          font-size: 12px;
        }
        ...
        @media (max-width: 480px) {
          ...
          .dp-enquire { font-size: 10px; letter-spacing: 0.22em; }
        }
      `}</style>
        ...
        <div className="dp-layout">
          ...
          >
            ...
            <div className="dp-main-img-wrap">
              ...
              {product.images.length > 1 && (
                <>
                  <button className="dp-img-prev" onClick={() => setImgIdx(p => (p - 1 + product.images.length) % product.images.length)}>
                    <ChevronLeft size={16} />
                  </button>
                  <button className="dp-img-next" onClick={() => setImgIdx(p => (p + 1) % product.images.length)}>
            ...
            <div className="dp-thumbs">
              {product.images.map((src, i) => (
                <div
                  key={i}
                  className={`dp-thumb${i === imgIdx ? " dp-thumb-on" : ""}`}
                  onClick={() => setImgIdx(i)}
                >
          ...
          >
            ...
            <div className="dp-meta">
              ...
              {product.badge && (
                <span className={`dp-badge ${badgeClass[product.badge] ?? ""}`}>{product.badge}</span>
              )}
            </div>
            ...
            <div className="dp-price-row">
              ...
              {product.originalPrice && (
                <span className="dp-price-orig">${product.originalPrice.toLocaleString()}</span>
              )}
              {product.originalPrice && (
            ...
            <div className="dp-actions">
              <button
                className={`dp-enquire${enquired ? " dp-enquire-done" : ""}`}
                onClick={handleEnquire}
                disabled={enquiring || enquired}
              >
            ...
            <div className="dp-strips">
              <div className="dp-strip">
                <Truck size={16} className="dp-strip-icon" />
                <div>
                  <p className="dp-strip-title">Delivery</p>
                  <p className="dp-strip-desc">{product.deliveryInfo}</p>
                </div>
              </div>
              <div className="dp-strip">
                <Shield size={16} className="dp-strip-icon" />
                <div>
                  <p className="dp-strip-title">Care Instructions</p>
                  <p className="dp-strip-desc">{product.careInstructions}</p>
                </div>
              </div>
              <div className="dp-strip">
                <Award size={16} className="dp-strip-icon" />
                <div>
                  <p className="dp-strip-title">Authenticity</p>
                  <p className="dp-strip-desc">Every piece comes with a certificate of authenticity, gemological report where applicable, and our lifetime craftsmanship guarantee.</p>
                </div>
              </div>
              <div className="dp-strip">
                <RotateCcw size={16} className="dp-strip-icon" />
                <div>
                  <p className="dp-strip-title">Returns</p>
                  <p className="dp-strip-desc">30-day returns on unworn items in original packaging. Custom and engraved pieces are non-returnable.</p>
                </div>
              </div>
            </div>
