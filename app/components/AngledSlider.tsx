...
export default function AngledSlider({ products }: { products?: Array<{ _id: string; name: string; slug: string; images: string[]; price: number }> }) {
  const x          = useMotionValue(0);
  const [paused, setPaused] = useState(false);
  const animRef    = useRef<ReturnType<typeof animate> | null>(null);
  
  const slides = products && products.length > 0
    ? products.map(p => ({ id: p._id, url: p.images[0], tag: p.name, name: p.name }))
    : [
