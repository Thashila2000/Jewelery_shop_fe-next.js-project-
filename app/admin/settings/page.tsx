...
const sectionSub: React.CSSProperties = {
  fontSize: 11, color: "#a08c5b", margin: "0 0 20px", fontWeight: 500,
};
  ...
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, padding: "13px 0", borderBottom: "0.5px solid #f5f3ee" }}>
      <div style={{ minWidth: 0 }}>
        <p style={{ fontSize: 13, fontWeight: 600, color: "#1a1109", margin: 0 }}>{label}</p>
        {sub && <p style={{ fontSize: 11, color: "#bbb", margin: "2px 0 0", lineHeight: 1.4 }}>{sub}</p>}
      </div>
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        background: active ? "rgba(177,141,43,0.1)" : "transparent",
    ...
    >
      <div style={{
        ...
        background: active ? "rgba(177,141,43,0.15)" : "#f5f3ee",
        display: "flex", alignItems: "center", justifyContent: "center",
        transition: "background 0.15s",
      }}>
        <Icon size={15} color={active ? "#b18d2b" : "#a08c5b"} />
      </div>
      <span style={{ fontSize: 13, fontWeight: active ? 700 : 500, color: active ? "#1a1109" : "#666", flex: 1 }}>{label}</span>
      {badge !== undefined && badge > 0 && (
        <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 6px", borderRadius: 10, background: "#fff1f2", color: "#9f1239", border: "0.5px solid #fda4af" }}>{badge}</span>
      )}
      <ChevronRight size={13} color={active ? "#b18d2b" : "#ccc"} />
    </motion.button>
  );
}
    ...
    <motion.div variants={panelVariants} initial="hidden" animate="visible" exit="exit" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
          ...
          <div>
            <p style={{ fontSize: 15, fontWeight: 800, color: "#1a1109", margin: 0 }}>Samantha De Silva</p>
            <p style={{ fontSize: 11, color: "#a08c5b", margin: "2px 0 0", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase" }}>Store Manager</p>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          {["Current Password", "New Password", "Confirm New Password"].map((label, i) => (
            <div key={label} style={{ gridColumn: i === 2 ? "1 / -1" : "auto", position: "relative" }}>
      ...
      <div style={cardStyle}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <div>
            ...
            <select defaultValue="Asia/Colombo" onFocus={onFocus} onBlur={onBlur} style={{ ...inputStyle, appearance: "none" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          {["Store Logo", "Favicon"].map(label => (
            <div key={label}>
              <label style={labelStyle}>{label}</label>
              <motion.div
                whileHover={{ borderColor: "#b18d2b", background: "#faf8f3" } as any}
...
function NotificationsPanel() {
  ...
  const groups = [
    {
      title: "Orders", sub: "Alerts related to customer orders and fulfilment.",
      rows: [
        ["newOrder",     "New order placed",             "Notify when a new order is received"],
        ["orderShipped", "Order dispatched",             "Notify when an order is marked as shipped"],
      ],
    },
    {
      title: "Inventory", sub: "Stock and product alerts.",
      rows: [
        ["lowStock",     "Low stock warning",            "Alert when any product falls below 3 units"],
      ],
    },
    {
      title: "Client Activity", sub: "Incoming communications and bookings.",
      rows: [
        ["newInquiry",      "New inquiry received",      "Notify when a client submits an inquiry"],
        ["newReview",       "New review submitted",      "Notify on all new product reviews"],
        ["newAppointment",  "Appointment booked",        "Notify when a new appointment is scheduled"],
      ],
    },
    {
      title: "Reports", sub: "Scheduled performance summaries.",
      rows: [
        ["dailyReport",  "Daily sales summary",          "Receive a daily revenue and order digest"],
        ["weeklyReport", "Weekly analytics report",      "Receive a full weekly performance report"],
      ],
    },
    {
      title: "Delivery Channels", sub: "How you receive notifications.",
      rows: [
        ["smsAlerts",    "SMS notifications",            "Receive critical alerts via SMS"],
        ["pushAlerts",   "Push notifications",           "Browser and mobile push notifications"],
      ],
    },
  ];
  ...
  return (
    <motion.div variants={panelVariants} initial="hidden" animate="visible" exit="exit" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <p style={sectionSub}>Choose what you're notified about and how.</p>
      </div>
  ...
  return (
    <motion.div variants={panelVariants} initial="hidden" animate="visible" exit="exit" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <p style={sectionSub}>Manage two-factor authentication, active sessions and account access.</p>
      </div>
      ...
      <div style={cardStyle}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: "#666", lineHeight: 1.5, margin: 0 }}>Add an extra layer of security. A verification code will be required at each login.</p>
          </div>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
          {sessions.map((s, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: i < sessions.length - 1 ? "0.5px solid #f5f3ee" : "none", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  <p style={{ fontSize: 13, fontWeight: 700, color: "#1a1109", margin: 0 }}>
                    {s.device}
                  ...
                  <p style={{ fontSize: 11, color: "#bbb", margin: "2px 0 0" }}>{s.location} · {s.time}</p>
                </div>
              </div>
              {!s.current && (
        ...
        <motion.button
          whileHover={{ background: "#fff1f2" } as any} whileTap={{ scale: 0.96 }}
      ...
      <div style={{ ...cardStyle, border: "0.5px solid #fda4af", background: "#fff9f9" }}>
        ...
        <p style={{ fontSize: 12, color: "#b45309", lineHeight: 1.5, margin: "0 0 16px" }}>These actions are irreversible. Please proceed with caution.</p>
...
function ShippingPanel() {
  ...
  return (
    <motion.div variants={panelVariants} initial="hidden" animate="visible" exit="exit" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        <p style={sectionTitle}>Shipping & Delivery</p>
        <p style={sectionSub}>Configure delivery zones, rates and packaging options.</p>
      </div>
      ...
      <div style={cardStyle}>
          ...
          {zones.map((z, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: i < zones.length - 1 ? "0.5px solid #f5f3ee" : "none", flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 120 }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: "#1a1109", margin: 0 }}>{z.name}</p>
                <p style={{ fontSize: 11, color: "#bbb", margin: "2px 0 0" }}>{z.time} · {z.rate}</p>
              </div>
              <div style={{
                fontSize: 9, fontWeight: 800, padding: "2px 8px", borderRadius: 5,
                background: z.active ? "#f0fdf4" : "#f5f3ee",
                color: z.active ? "#166534" : "#bbb",
                border: `0.5px solid ${z.active ? "#bbf7d0" : "#e8e3d8"}`,
        ...
        <motion.button
          whileHover={{ background: "#faf8f3", borderColor: "#b18d2b" } as any}
          whileTap={{ scale: 0.96 }}
  ...
  return (
    <motion.div variants={panelVariants} initial="hidden" animate="visible" exit="exit" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <p style={sectionSub}>Customise the look and feel of your admin dashboard.</p>
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
          {(["light", "dark", "system"] as const).map(t => (
            <motion.div
              ...
              onClick={() => setTheme(t)}
              style={{
                border: `${theme === t ? "1.5px" : "0.5px"} solid ${theme === t ? "#b18d2b" : "#e8e3d8"}`,
                borderRadius: 10, padding: "14px 12px", cursor: "pointer",
                background: theme === t ? "rgba(177,141,43,0.05)" : "#faf8f3",
          ...
          {accents.map(c => (
            <motion.div
              ...
              style={{
                ...
                cursor: "pointer", border: accent === c ? `2.5px solid #1a1109` : "2.5px solid transparent",
                display: "flex", alignItems: "center", justifyContent: "center",
                boxShadow: accent === c ? `0 0 0 3px ${c}44` : "none",
                transition: "all 0.15s",
              }}
            >
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28, gap: 12, flexWrap: "wrap" }}
      >
        <div style={{ transform: "translateY(-10px)" }}>
          <h1 style={{
            fontSize: 31, fontWeight: 750, color: "#1a1109",
            marginTop: 0, marginBottom: 0, marginLeft: 0, marginRight: 0,
            letterSpacing: "-0.03em", lineHeight: 1, fontFamily: "sans-serif",
          }}>
            Settings
          </h1>
          <p style={{
      ...
      <motion.div
        variants={itemVariants}
        style={{ display: "grid", gridTemplateColumns: "clamp(180px, 22%, 240px) 1fr", gap: 18, alignItems: "start" }}
      >
        {/* Sidebar nav */}
        <div style={{ background: "#fff", border: "0.5px solid #e8e3d8", borderRadius: 14, padding: "10px 10px", position: "sticky", top: 24 }}>
          <p style={{ fontSize: 9, fontWeight: 700, color: "#a08c5b", textTransform: "uppercase", letterSpacing: "0.18em", margin: "6px 6px 10px", display: "block" }}>
            Configuration
          </p>
